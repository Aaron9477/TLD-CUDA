...
//    imAcq->method = IMACQ_CAM;
    ...
    imAcq->imgPath = "/home/ubuntu/WYZ/abc.avi";
...
void imAcqInit(ImAcq *imAcq) {
    if (imAcq->method == IMACQ_CAM) {
        ...
        imAcq->capture = cvCaptureFromCAM(imAcq->camNo); //cvCaptureFromCAM初始化从摄像头中获取视频
...
IplImage *imAcqGrab(CvCapture *capture) {
    ...
    frame = cvQueryFrame(capture);//cvQueryFrame从摄像头或者文件中抓取并返回一帧
